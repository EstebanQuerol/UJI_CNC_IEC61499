...
COpcConnection::COpcConnection(const char *pa_acHost, const char *pa_acServerName) :
    m_acHost(pa_acHost), m_acServerName(pa_acServerName), m_bIsConnected(false), m_nGroupCount(0), m_eConnectionEvent(e_Disconnected), m_bBlockingConnect(false){
...
void COpcConnection::addGroup(const char* pa_acGroupName, unsigned long pa_nReqUpdateRate, float pa_nDeadBand, IComCallback* pa_pComCallback){
  m_oSync.lock();
