	...
	if(pa_nEIID < 1 && pa_nEIID > -1){
		...
		for(int input_index = 0; input_index < m_nDInputs; input_index++){
			...
			if(pDataInput->getDataTypeID() == CIEC_ANY::e_STRING || pDataInput->getDataTypeID() == CIEC_ANY::e_WSTRING){
				...
				nStringLength = (int)(static_cast<CIEC_ANY_STRING*>(pDataInput))->length();

				if(input_index == 0){
					//initialize output with first value to append
					(static_cast<CIEC_STRING*>(pDataOutput))->assign((static_cast<CIEC_ANY_STRING*>(pDataInput))->getValue(), (TForteUInt16) nStringLength);
				}
				else{
					//append string value to output
					(static_cast<CIEC_STRING*>(pDataOutput))->append((static_cast<CIEC_ANY_STRING*>(pDataInput))->getValue(), (TForteUInt16) nStringLength);
				}
			}
			//no string data type (use method toString)
				...
				if(pDataInput->getDataTypeID() == CIEC_ANY::e_ARRAY){
					...
					//DEVLOG_DEBUG("Array Required Bytes: %d;\n", nRequiredBytes);
				}
				else{
					//size for single data values
					nRequiredBytes = scm_maxStringBufSize;
					//DEVLOG_DEBUG("Normal Required Bytes: %d;\n", nRequiredBytes);
				}
				...
				if((static_cast<CIEC_STRING*>(pDataOutput))->length() != static_cast<TForteUInt16>(0) && input_index != 0){
					...
					//DEVLOG_DEBUG("CurrentLength: %d; FreeBytes: %d;\n", nCurrentLength, nBytesFree);		
				...
				if((static_cast<CIEC_STRING*>(pDataOutput))->getCapacity() - nCurrentLength >= nRequiredBytes){
					...
					nUsedBytes = pDataInput->toString(((static_cast<CIEC_STRING*>(pDataOutput))->getValue()) + nCurrentLength, scm_maxStringBufSize);
					//DEVLOG_DEBUG("nUsedBytes: %d;\n", nUsedBytes);
