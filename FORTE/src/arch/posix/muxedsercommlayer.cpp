...
CEventSourceFB *CMuxedSerCommLayer::onComEvent(){
  int nReadCount = read(m_nFD, &m_acRecvBuffer[m_unBufFillSize], cg_unIPLayerRecvBufferSize - m_unBufFillSize);
...
EComResponse CMuxedSerCommLayer::processComEvent(){
...
EComResponse CMuxedSerCommLayer::openConnection(char *pa_acLayerParameter){
  ...
  if(0 != acPort){
    *acPort = '\0';
    ++acPort;
    m_unSerMuxId = forte::core::util::strtol(acPort, 0, 10);
...
CEventSourceFB *CMuxedSerCommLayer::CMuxedSerPortsManager::SSerPortEntry::onComEvent(){
  CEventSourceFB *poRetVal = 0;
  char nID;

  int nReadCount = read(m_nFD, &nID, 1);
...
EComResponse CMuxedSerCommLayer::CMuxedSerPortsManager::SSerPortEntry::processComEvent(){
  return e_Nothing;
}
